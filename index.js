window.addEventListener('DOMContentLoaded', async () => {
  ...
  function renderCards(list) {
    ...
    list.forEach(d => {
      ...
      card.className = 'card';
      card.innerHTML = `<h3>${d.district}</h3><button class="toggle-data">Show Data ▼</button><div class="raw-data" style="display:none;"><p>Total Medicaid: ${d.total_medicaid.toLocaleString()}</p><p>SNAP Households: ${d.snap_households.toLocaleString()}</p><p>Job Losses: ${d.jobs_risk.toLocaleString()}</p></div>`;
  ...
  stateSelect.addEventListener('change', () => {
    ...
    if (swings.length) {
      ...
      swings.forEach(d => { selectHTML += `<option value="${d.district}">${d.district}</option>`; });
      selectHTML += `</select>`;
